...
for LOOP = 1:LOOPNUM
    ...
    for itemOrder = 1:length(itemRange)
        ...
        for epsilonOrder = 1:length(epsilonRange)
            ...
            RoundNumRecord(itemOrder,LOOP,epsilonOrder) = length(APPCOMBIDLOG);
...
%% Figure 4: User delay with different epsilon
% centralMean = mean(centralRecord,1);
% epsilonMean = mean(PAUSERecord,2);
epsilonMean = mean(RoundNumRecord,2);
...
grid on
% legend('Central','Epsilon=1','Epsilon=5','Epsilon=15','Epsilon=25')
legend('Epsilon=1','Epsilon=5','Epsilon=15','Epsilon=25')
set(gca,'FontName','times new Roman');
xlabel('Items','FontName','Times New Roman','FontSize',11)
ylabel('User delay','FontName','Times New Roman','FontSize',11)
